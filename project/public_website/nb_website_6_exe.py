...
def get_exe_name_list(script):
    return re.findall(r'\/(.*\.exe)', script)
...
for u in url_script:
    ...
    for script in u['jsScript']:
        ...
        extend_exe(exe_in_comment, comment_string)

        script_string = remove_comment(script)
        extend_exe(exe_in_non_comment, script_string)

    url_activated[u['url']]['exe_in_comment'] = exe_in_comment
    url_activated[u['url']]['exe_in_non_comment'] = exe_in_non_comment
